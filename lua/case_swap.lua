function replace_word_under_cursor(new_word)
    ...
    vim.fn.cursor('.', start_pos + string.len(new_word))
    return replaced_word
end


-- fooBooBazBarFloo
vim.api.nvim_create_user_command('CamelToSnakeCase',
function()
  local word = vim.fn.expand('<cword>')
  local new_word = word:gsub("%u", function(c) return "_" .. c:lower() end)
  replace_word_under_cursor(new_word)
end, {})

-- fooBooBazBarFoo
vim.api.nvim_create_user_command('SnakeToCamelCase',
function()
  local word = vim.fn.expand('<cword>')
  local new_word = word:gsub("_(%a)", function(c) return c:upper() end)
  replace_word_under_cursor(new_word)
end, {})

vim.keymap.set('n', 'gtc', ':SnakeToCamelCase<CR>')
vim.keymap.set('n', 'gts', ':CamelToSnakeCase<CR>')
